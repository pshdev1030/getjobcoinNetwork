...
public class HeartBeatReceiverRunnable implements Runnable{
    ...
    public HeartBeatReceiverRunnable (Socket toClient, HashMap<Peer, Date> peerList, int localPort) {
        	...
        	while (true) {
            	...
            	switch (tokens[0]) {
                	...
                	case "SI":
                		...
                		peerList.put(serverInQuestion, new Date());
                		peerList.put(originator, new Date());
                		this.removeUnresponsive();
    ...
    public void removeUnresponsive() {
        ...
        for (Peer server: peerList.keySet()) {
            if (new Date().getTime() - peerList.get(server).getTime() > 4000) {
            	peerList.remove(server);
